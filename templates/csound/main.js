...
async function setup(sampleRate) {
  const ready = new Promise(resolve => (Module.onRuntimeInitialized = resolve))
  Module = libcsound(Module)
  await ready
  Csound = {
      new: Module.cwrap('CsoundObj_new', ['number'], null),
      compileCSD: Module.cwrap('CsoundObj_compileCSD', ['number'], ['number', 'string']),
  
      reset: Module.cwrap('CsoundObj_reset', null, ['number']),
      getOutputBuffer: Module.cwrap('CsoundObj_getOutputBuffer', ['number'], ['number']),
      getInputBuffer: Module.cwrap('CsoundObj_getInputBuffer', ['number'], ['number']),
      getKsmps: Module.cwrap('CsoundObj_getKsmps', ['number'], ['number']),
      performKsmps: Module.cwrap('CsoundObj_performKsmps', ['number'], ['number']),
      // Ack, not exported.
      // performBuffer: Module.cwrap('CsoundObj_performBuffer', ['number'], ['number']),
  
      getInputChannelCount: Module.cwrap('CsoundObj_getInputChannelCount', ['number'], ['number']),
      getOutputChannelCount: Module.cwrap('CsoundObj_getOutputChannelCount', ['number'], ['number']),
      getZerodBFS: Module.cwrap('CsoundObj_getZerodBFS', ['number'], ['number']),
      compileOrc: Module.cwrap('CsoundObj_compileOrc', 'number', ['number', 'string']),
      setOption: Module.cwrap('CsoundObj_setOption', null, ['number', 'string']),
      prepareRT: Module.cwrap('CsoundObj_prepareRT', null, ['number']),
      setMidiCallbacks: Module.cwrap('CsoundObj_setMidiCallbacks', null, ['number']),
  }
  csObj = Csound.new()
  ...
  Csound.setOption(csObj, "--nchnls_i=0"); 
  Csound.compileCSD(csObj, "main.csd")
  const ksmps = Csound.getKsmps(csObj)
  const channels = Csound.getOutputChannelCount(csObj)
  csBufferLength = ksmps * channels
  csBuffer = new Float32Array(Module.HEAP8.buffer, Csound.getOutputBuffer(csObj), csBufferLength)
  scale = 1 / Csound.getZerodBFS(csObj)
}

function process(output) {
  output.set(csBuffer.subarray(csBufferLength - offset))
  let i = offset
  if (done) {
      return i
  }
  while (i < output.length - csBufferLength) {
    done = Csound.performKsmps(csObj)
      output.set(csBuffer, i)
      i += csBufferLength
      if (done) {
          return i
      }
  ...
  output.set(csBuffer.subarray(0, output.length - i), i)
  if (done) {
    return output.length
  }
  offset = csBufferLength - (output.length - i)
  // TODO: Simplify structure here? (One pass that generates, scales, & copies.)
...
Module.FS.createPath("/", "assets", true, true);
      ...
      Module.FS.createDataFile(this.name, null, byteArray, true, true, true)
